...
### 2026-08-02: Unconfirmed drafts from the first Stage 2 batch (not yet in the glossary)

**Decided by:** nobody yet. These are **proposals**, raised by the worker that translated the
`rock-paper-scissors` and `rainbow` exercise message catalogs (those catalogs have no API
path, so they stay on the subagent path even though es-419 runs on DeepSeek). They are
recorded here so the wording used in those two exercises is traceable, and so a native
speaker can object. **None of them has been written into `glossary.md`**, so no other pass is
bound by them yet.
