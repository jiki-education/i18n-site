...
- The frame of the house (the big rectangle) should be `60` wide and `40` <define>`height`</define>. It should have its top-left corner at `20`x`50`.
...
- The windows are both the same size, with a <define>`width`</define> of `12` and a `height` of `13`. They both sit `5` from the top of the house frame, and `10` inset from the sides.
